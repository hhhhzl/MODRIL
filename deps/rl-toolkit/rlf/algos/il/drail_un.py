...
from rlf.algos.ddpm import MLPDiffusionCustomize
...
class Discriminator(nn.Module):
    def __init__(self, state_dim, action_dim, args, base_net, num_units=128):
        ...
        d_model = MLPDiffusionCustomize(n_steps, input_dim, num_units=num_units, depth=self.args.discrim_depth).to(
            self.args.device)
...
class DRAIL_UNDiscrim(BaseIRLAlgo):
    ...
    def _create_discrim(self):
        ...
        discrim = self.get_discrim(base_net.output_shape[0], ac_dim, self.args, base_net,
                                   num_units=self.args.discrim_num_unit)
